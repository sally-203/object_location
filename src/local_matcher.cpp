...
void LocalMatcher::ExtractISSKeypoints(bool flag,
                                       const IssParameters& iss_param) {
...
void LocalMatcher::ExtractDownSamplingKeypoints(bool flag, double radius) {
  ...
  if (flag) {
    scene_keypoints_ = keypoints;
    std::cout << "Scene Keypoints Size: " << scene_keypoints_->size()
              << std::endl;
  } else {
    model_keypoints_ = keypoints;
    std::cout << "Model Keypoints Size: " << model_keypoints_->size()
              << std::endl;
  }
...
void LocalMatcher::PFHMatch(const PfhParameters& pfh_param) {
  ...
  CalculatePfhDescri(scene_keypoints_, pfh_param.pfh_radius, scene_descriptors);
  CalculatePfhDescri(model_keypoints_, pfh_param.pfh_radius, model_descriptors);
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).histogram[0])) {
      ...
      // std::cout << "squaredDistance[0]: " << squaredDistances[0] <<
      // std::endl;
...
void LocalMatcher::CalculatePfhDescri(
    const PXYZS::Ptr cloud, double pfh_radius,
...
void LocalMatcher::FPFHMatch(const FpfhParameters& fpfh_param) {
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).histogram[0])) {
      ...
      if (neighborCount == 1 &&
          squaredDistances[0] < fpfh_param.distance_thre) {
...
void LocalMatcher::CalculateFpfhDescri(
    const PXYZS::Ptr cloud, double fpfh_radius,
...
void LocalMatcher::RSDMatch(const RsdParameters& rsd_parameters) {
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    // std::cout << squaredDistances[0] << std::endl;
    if (neighborCount == 1 &&
        squaredDistances[0] < rsd_parameters.distance_thre) {
      pcl::Correspondence correspondence(neighbors[0], static_cast<int>(i),
                                         squaredDistances[0]);
      correspondences->push_back(correspondence);
    }
  ...
  pose.setMaxCorrespondenceDistance(
      rsd_parameters.common_params.corres_distance);
...
void LocalMatcher::CalculateRsdDescri(
    const PXYZS::Ptr cloud, double rsd_radius, double plane_radius,
...
void LocalMatcher::DSC3Match(const Dsc3Parameters& dsc3_param) {
  ...
  CalculateDscDescri(scene_keypoints_, dsc3_param.dsc_radius,
                     dsc3_param.minimal_radius, dsc3_param.point_density_radius,
                     scene_descriptors);
  CalculateDscDescri(model_keypoints_, dsc3_param.dsc_radius,
                     dsc3_param.minimal_radius, dsc3_param.point_density_radius,
                     model_descriptors);
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).descriptor[0])) {
      ...
      // std::cout << squaredDistances[0] << std::endl;
      if (neighborCount == 1 &&
          squaredDistances[0] < dsc3_param.distance_thre) {
        pcl::Correspondence correspondence(neighbors[0], static_cast<int>(i),
                                           squaredDistances[0]);
        correspondences->push_back(correspondence);
      }
...
void LocalMatcher::CalculateDscDescri(
    const PXYZS::Ptr cloud, double dsc_radius, double minimal_radius,
...
void LocalMatcher::USCMatch(const UscParameters& usc_param) {
  ...
  CalculateUscDescri(scene_keypoints_, usc_param.usc_radius,
                     usc_param.minimal_radius, usc_param.point_density_radius,
                     usc_param.local_radius, scene_descriptors);
  CalculateUscDescri(model_keypoints_, usc_param.usc_radius,
                     usc_param.minimal_radius, usc_param.point_density_radius,
                     usc_param.local_radius, model_descriptors);
...
void LocalMatcher::CalculateUscDescri(
    const PXYZS::Ptr cloud, double usc_radius, double minimal_radius,
...
void LocalMatcher::SHOTMatch(const ShotParameters& shot_param) {
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).descriptor[0])) {
      ...
      // std::cout << squaredDistances[0] << std::endl;
      if (neighborCount == 1 &&
          squaredDistances[0] < shot_param.distance_thre) {
        pcl::Correspondence correspondence(neighbors[0], static_cast<int>(i),
                                           squaredDistances[0]);
        correspondences->push_back(correspondence);
      }
...
void LocalMatcher::CalculateShotDescri(
    const PXYZS::Ptr cloud, double shot_radius,
...
void LocalMatcher::SIMatch(const SpinParameters& spin_param) {
  ...
  CalculateSiDescri(scene_keypoints_, spin_param.si_radius,
                    spin_param.image_width, spin_param.normal_radius,
                    scene_descriptors);
  CalculateSiDescri(model_keypoints_, spin_param.si_radius,
                    spin_param.image_width, spin_param.normal_radius,
                    model_descriptors);
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).histogram[0])) {
      ...
      // std::cout << squaredDistances[0] << std::endl;
      if (neighborCount == 1 &&
          squaredDistances[0] < spin_param.distance_thre) {
        pcl::Correspondence correspondence(neighbors[0], static_cast<int>(i),
                                           squaredDistances[0]);
        correspondences->push_back(correspondence);
      }
...
void LocalMatcher::CalculateSiDescri(
    const PXYZS::Ptr cloud, double si_radius, int image_width,
...
void LocalMatcher::ROPSMatch(const RopsParameters& rops_param) {
  ...
  CalculateRopsDescri(scene_keypoints_, rops_param.rops_radius,
                      rops_param.num_partions_bins, rops_param.num_rotations,
                      rops_param.support_radius, rops_param.normal_radius,
                      scene_descriptors);
  ...
  for (size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (std::isfinite(scene_descriptors->at(i).histogram[0])) {
      ...
      // std::cout << squaredDistances[0] << std::endl;
      if (neighborCount == 1 &&
          squaredDistances[0] < rops_param.distance_thre) {
        pcl::Correspondence correspondence(neighbors[0], static_cast<int>(i),
                                           squaredDistances[0]);
        correspondences->push_back(correspondence);
      }
...
void LocalMatcher::CalculateRopsDescri(
    const PXYZS::Ptr cloud, double rops_radius, int num_partions_bins,
    int num_rotations, double support_radius, double normal_radius,
...
void LocalMatcher::AbsolueAccuracyEstimate(Eigen::Matrix4f& true_pose) {
  ...
  std::cout << "translation error: "
            << (trans1 - trans2).array().abs().transpose() << std::endl;
